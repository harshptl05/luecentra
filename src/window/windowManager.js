...
/**
 * Get or create the in-app window that shows the full Pulse web app (localhost site from codebase).
 * This is your own copy: personalization, settings, Firebase/Google auth all work here.
 */
function getOrCreateWebAppWindow() {
    let webapp = windowPool.get('webapp');
    if (webapp && !webapp.isDestroyed()) return webapp;
    const webUrl = process.env.pickleglass_WEB_URL || 'http://localhost:3000';
    webapp = new BrowserWindow({
        width: 1100,
        height: 800,
        minWidth: 800,
        minHeight: 600,
    ...
    webapp.on('closed', () => windowPool.delete('webapp'));
    windowPool.set('webapp', webapp);
    return webapp;
}

/**
 * Open the full localhost site (personalization, settings, etc.) in the in-app window.
 * Uses the exact same codebase — Firebase and Google auth work here.
 */
function openWebAppWindow(path = '/') {
    const webUrl = process.env.pickleglass_WEB_URL || 'http://localhost:3000';
    const fullUrl = path.startsWith('http') ? path : `${webUrl}${path.startsWith('/') ? path : '/' + path}`;
    const webapp = getOrCreateWebAppWindow();
    webapp.loadURL(fullUrl);
    webapp.show();
    webapp.focus();
    console.log('[WindowManager] Opening Pulse site:', fullUrl);
}

const openLoginPage = () => {
    openWebAppWindow('/personalize');
};
...
function createWindows() {
    ...
    // Ensure window is visible on creation
    header.show();
