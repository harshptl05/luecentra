...
const licensingService = require('../features/common/services/licensingService');
...
module.exports = {
  ...
  initialize() {
    ...
    ipcMain.handle('start-firebase-auth', async () => {
        // Open login in the full Pulse site window (same codebase: personalization, settings, Firebase/Google auth)
        const { openWebAppWindow } = require('../window/windowManager');
        openWebAppWindow('/login?mode=electron');
        return { success: true };
    });
    ...
    // Licensing (device binding + subscription)
    ipcMain.handle('licensing:getDeviceId', () => licensingService.getOrCreateDeviceId());
    ipcMain.handle('licensing:ensureActivated', () => licensingService.ensureActivated());
    ipcMain.handle('licensing:loginWithCredentials', (e, { email, password }) => licensingService.loginWithCredentials(email, password));
    ipcMain.handle('licensing:loginWithTokens', (e, { accessToken, refreshToken }) => licensingService.loginWithTokens(accessToken, refreshToken));
    ipcMain.handle('licensing:logout', () => licensingService.logout());
    ipcMain.handle('licensing:isLicensed', () => licensingService.isLicensed());
    ipcMain.handle('licensing:getAccessToken', () => licensingService.getAccessToken());
    ...
    // Meeting Coach
    ipcMain.handle('meeting-coach:trigger-answer', async (event, question) => {
      try {
        const result = await listenService.triggerMeetingCoachAnswer(question);
        return result;
      } catch (error) {
        console.error('[FeatureBridge] meeting-coach:trigger-answer failed', error.message);
        return { success: false, error: error.message };
      }
    });

    // Verify preset access for meeting coach
    ipcMain.handle('meeting-coach:verify-preset', async () => {
      try {
        const result = await listenService.meetingCoachService.verifyPresetAccess();
        return result;
      } catch (error) {
        console.error('[FeatureBridge] meeting-coach:verify-preset failed', error.message);
        return { success: false, error: error.message };
      }
    });

    // Meeting Context Management (deprecated - using presets instead)
    // Note: Context management is now handled via the Personalize / Meeting Notes preset system
    ipcMain.handle('meeting-coach:get-all-contexts', async () => {
      return { success: false, error: 'Context management is now handled via Presets. Use Settings > Personalize / Meeting Notes instead.' };
    });

    ipcMain.handle('meeting-coach:create-context', async () => {
      return { success: false, error: 'Context management is now handled via Presets. Use Settings > Personalize / Meeting Notes instead.' };
    });

    ipcMain.handle('meeting-coach:update-context', async () => {
      return { success: false, error: 'Context management is now handled via Presets. Use Settings > Personalize / Meeting Notes instead.' };
    });

    ipcMain.handle('meeting-coach:set-active-context', async () => {
      return { success: false, error: 'Context management is now handled via Presets. Use Settings > Personalize / Meeting Notes instead.' };
    });

    ipcMain.handle('meeting-coach:delete-context', async () => {
      return { success: false, error: 'Context management is now handled via Presets. Use Settings > Personalize / Meeting Notes instead.' };
    });

    // File selection for context
    const { dialog } = require('electron');
    ipcMain.handle('meeting-coach:select-files', async () => {
      try {
        ...
        console.error('[FeatureBridge] meeting-coach:select-files failed', error.message);
