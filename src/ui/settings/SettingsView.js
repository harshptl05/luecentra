...
export class SettingsView extends LitElement {
    ...
    async handleSaveKey(provider) {
        ...
        if (result.success) {
            // Save the key after validation succeeds
            const saveResult = await window.api.settingsView.saveApiKey(key, provider);
            if (saveResult && saveResult.success) {
                await this.refreshModelData();
            } else {
                alert(`Failed to save ${provider} key: ${saveResult?.error || 'Unknown error'}`);
                input.value = this.apiKeys[provider] || '';
            }
        } else {
            alert(`Failed to validate ${provider} key: ${result.error}`);
            input.value = this.apiKeys[provider] || '';
        }
        this.saving = false;
    }
    ...
    getMainShortcuts() {
        return [
            ...
            { name: 'Trigger Meeting Coach', accelerator: this.shortcuts.triggerMeetingCoach },
