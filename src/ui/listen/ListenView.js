...
import './meetingCoach/MeetingCoachView.js';
import './meetingCoach/ContextManagerView.js';
...
.hljs-tag {
            ...
            window.api.listenView.onSessionStateChanged((event, { isActive }) => {
                ...
                if (!wasActive && isActive) {
                    ...
                    this.updateComplete.then(() => {
                        ...
                        const coachView = this.shadowRoot.querySelector('meeting-coach-view');
                        if (sttView) sttView.resetTranscript();
                        if (summaryView) summaryView.resetAnalysis();
                        // Coach view doesn't need reset as it manages its own state
                    });
                    this.requestUpdate();
                }
                if (wasActive && !isActive) {
        ...
        this.updateComplete
            .then(() => {
                ...
                if (this.viewMode === 'transcript') {
                    ...
                    activeContent = this.shadowRoot.querySelector('meeting-coach-view');
                } else if (this.viewMode === 'context') {
                    activeContent = this.shadowRoot.querySelector('context-manager-view');
                } else {
    ...
    toggleViewMode() {
        // Cycle through: insights -> transcript -> coach -> context -> insights
    ...
    render() {
        ...
        return html`
            <div class="assistant-container">
                ...
                <meeting-coach-view 
                    .isVisible=${this.viewMode === 'coach'}
                ></meeting-coach-view>

                <context-manager-view 
                    .isVisible=${this.viewMode === 'context'}
                ></context-manager-view>
            </div>
        `;
    }
