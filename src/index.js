...
function setupProtocolHandling() {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        console.log('[Protocol] Second instance prevented - focusing existing window');
        console.log('[Protocol] Second instance command line:', commandLine);
        
        // Always focus the main window first
        focusMainWindow();
        ...
        if (protocolUrl) {
            ...
            // Small delay to ensure window is focused before handling URL
            setTimeout(() => {
    ...
    app.on('open-url', (event, url) => {
        ...
        // Always focus the main window when handling deep links
        focusMainWindow();
...
app.whenReady().then(async () => {
    ...
    try {
        ...
        function attachAuthIntercept(win) {
            if (!win || !win.webContents || win.isDestroyed()) return;
            win.webContents.on('will-navigate', (event, url) => {
                if (!url || !url.startsWith('pickleglass://auth-success')) return;
                event.preventDefault();
                const wc = win.webContents;
                console.log('[Auth] Intercepted auth-success (full URL received)');
                (async () => {
                    try {
                        await handleCustomUrl(url);
                        const user = authService.getCurrentUser();
                        if (user && user.isLoggedIn) {
                            const profile = { uid: user.uid, display_name: user.displayName, email: user.email };
                            await wc.loadURL(`http://localhost:${WEB_PORT}/`);
                            await wc.executeJavaScript(`
                                localStorage.setItem('pickleglass_user', JSON.stringify(${JSON.stringify(profile)}));
                            `).catch(() => {});
                            await wc.loadURL(`http://localhost:${WEB_PORT}/personalize`);
                        } else {
                            wc.loadURL(`http://localhost:${WEB_PORT}/personalize`).catch(() => {});
                        }
                    } catch (err) {
                        console.error('[Auth] handleCustomUrl failed:', err);
                        wc.loadURL(`http://localhost:${WEB_PORT}/personalize`).catch(() => {});
                    }
                })();
            });
        }
        BrowserWindow.getAllWindows().forEach(attachAuthIntercept);
        app.on('browser-window-created', (_e, win) => attachAuthIntercept(win));
        ...
        try {
            switch (channel) {
                ...
                case 'set-active-preset':
                    // Adapter injects UID
                    result = await presetRepository.setActive(payload);
                    settingsService.notifyPresetUpdate('activated', payload);
                    break;
...
async function startWebStack() {
  ...
  const createBackendApp = require('../pulse_web/backend_node');
  const nodeApi = createBackendApp(eventBridge);

  const staticDir = app.isPackaged
    ? path.join(process.resourcesPath, 'out')
    : path.join(__dirname, '..', 'pulse_web', 'out');
  ...
  if (!fs.existsSync(staticDir)) {
    ...
    console.error(`Please run 'npm run build' inside the 'pulse_web' directory first.`);
    console.error(`============================================================`);
    app.quit();
    return;
  }
    ...
    try {
        ...
        autoUpdater.on('update-downloaded', (event, releaseNotes, releaseName, date, url) => {
            ...
            dialog.showMessageBox({
                ...
                message: `A new version of Pulse Glass (${releaseName}) has been downloaded. It will be installed the next time you launch the application.`,
