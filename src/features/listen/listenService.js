...
const MeetingCoachService = require('../meetingCoach/meetingCoachService');
...
class ListenService {
    constructor() {
        ...
        this.meetingCoachService = new MeetingCoachService();
    ...
    setupServiceCallbacks() {
        ...
        // Hook into STT updates for meeting coach (both partial and final)
        // This ensures speaker identification is always correct
        this.setupSttUpdateListener();
        ...
        // Meeting Coach service callbacks
        this.meetingCoachService.setCallbacks({
            onAnswerGenerated: (answer) => {
                this.sendToRenderer('meeting-coach:answer', answer);
            },
            onStatusUpdate: (status) => {
                this.sendToRenderer('meeting-coach:status', status);
            }
        });
    }
    ...
    sendToRenderer(channel, data) {
        ...
        // Also process STT updates for meeting coach (ensures speaker info is tracked)
        if (channel === 'stt-update' && data.speaker && this.meetingCoachService?.isActive) {
            // Process both partial and final transcripts for better context
            this.meetingCoachService.processTranscript(
                data.text, 
                data.speaker, 
                data.isFinal || false,
    ...
    setupSttUpdateListener() {
        // This ensures we catch all STT updates, including interim ones
        // The sendToRenderer method above will handle forwarding to meeting coach
    }
    ...
    async handleTranscriptionComplete(speaker, text) {
        ...
        // Process with meeting coach (for question detection and answer generation)
        // Pass both final and interim transcripts for better real-time updates
        if (this.meetingCoachService.isActive) {
            await this.meetingCoachService.processTranscript(text, speaker, true);
        }
    }

    /**
     * Handle interim transcript updates (for live transcript display)
     * Also updates meeting coach with partial transcripts for better context
     */
    async handleTranscriptionUpdate(speaker, text, isPartial) {
        // Process interim transcripts with meeting coach for better context
        // (but don't trigger answers on partial transcripts)
        if (this.meetingCoachService.isActive && isPartial) {
            await this.meetingCoachService.processTranscript(text, speaker, false);
        }
    }

    /**
     * Manually trigger meeting coach to generate an answer
     * Can be called via hotkey
     */
    async triggerMeetingCoachAnswer(question = null) {
        if (!this.meetingCoachService.isActive) {
            return { success: false, error: 'Meeting coach is not active. Start a listen session first.' };
        }

        try {
            await this.meetingCoachService.triggerManualAnswer(question);
            return { success: true };
        } catch (error) {
            console.error('[ListenService] Error triggering meeting coach answer:', error);
    ...
    async initializeNewSession() {
        try {
            ...
            // Start meeting coach service
            await this.meetingCoachService.start(this.currentSessionId);
    ...
    async closeSession() {
        try {
            ...
            // Stop meeting coach service
            this.meetingCoachService.stop();
