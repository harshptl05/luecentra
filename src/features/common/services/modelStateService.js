...
class ModelStateService extends EventEmitter {
    ...
    async handleLocalAIStateChange(service, state) {
        try {
            ...
            this.emit('state-updated', liveState);
        } catch (error) {
            // Silently handle EPIPE and other stream errors that can occur during shutdown
            if (error.code === 'EPIPE' || error.message?.includes('EPIPE')) {
                console.warn(`[ModelStateService] EPIPE error in handleLocalAIStateChange (likely during shutdown):`, error.message);
                return;
            }
            console.error(`[ModelStateService] Error in handleLocalAIStateChange:`, error);
        }
    }
