...
function setActive(id, uid) {
    const db = sqliteClient.getDb();
    
    try {
        // First, deactivate all presets for this user
        db.prepare(`UPDATE prompt_presets SET is_active = 0 WHERE uid = ?`).run(uid);
        
        // Then activate the selected preset (only if it belongs to the user or is default)
        const result = db.prepare(`UPDATE prompt_presets SET is_active = 1 WHERE id = ? AND (uid = ? OR is_default = 1)`).run(id, uid);
