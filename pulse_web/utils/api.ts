...
export const setActivePreset = async (id: string): Promise<void> => {
  if (isFirebaseMode()) {
    const uid = firebaseAuth.currentUser!.uid;
    await FirestorePromptPresetService.setActivePreset(uid, id);
  } else {
    const response = await apiCall(`/api/presets/${id}/activate`, { method: 'POST' });
