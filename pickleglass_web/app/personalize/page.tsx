...
import { getPresets, updatePreset, createPreset, setActivePreset, PromptPreset } from '@/utils/api'
...
export default function PersonalizePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (presetsData.length > 0) {
          // Find active preset first, then fallback to first user preset or first preset
          const activePreset = presetsData.find(p => p.is_active === 1);
          const firstUserPreset = presetsData.find(p => p.is_default === 0) || presetsData[0];
          const presetToSelect = activePreset || firstUserPreset;
          setSelectedPreset(presetToSelect);
          setEditorContent(presetToSelect.prompt);
        }
      } catch (error) {
  ...
  const handlePresetClick = async (preset: PromptPreset) => {
    ...
    // Activate the preset when clicked
    try {
      await setActivePreset(preset.id);
      // Update local state to reflect active status
      setAllPresets(prev => prev.map(p => ({ ...p, is_active: p.id === preset.id ? 1 : 0 })));
    } catch (error) {
            ...
            <div className="grid grid-cols-5 gap-4 mb-6">
              {allPresets.map((preset) => (
                ...
                >
                  ...
                  {preset.is_active === 1 && (
                    <div className="absolute top-2 left-2 bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full font-semibold">
                      Active
                    </div>
                  )}
