...
export class FirestorePromptPresetService {
  ...
  static async setActivePreset(uid: string, presetId: string) {
    // First, deactivate all user presets
    const presetsRef = collection(firestore, 'users', uid, 'promptPresets');
    const q = query(presetsRef);
    const querySnapshot = await getDocs(q);
    
    const deactivatePromises = querySnapshot.docs.map(docRef => {
      const presetRefToUpdate = doc(firestore, 'users', uid, 'promptPresets', docRef.id);
      return updateDoc(presetRefToUpdate, { is_active: 0 });
    });
    await Promise.all(deactivatePromises);
    
    // Then activate the selected preset
    const presetRef = doc(firestore, 'users', uid, 'promptPresets', presetId);
    await updateDoc(presetRef, { is_active: 1 });
  }
} 