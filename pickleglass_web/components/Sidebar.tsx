...
const SidebarComponent = ({ isCollapsed, onToggle, onSearchClick }: SidebarProps) => {
    ...
    const bottomItems = useMemo(
        () => [
            {
                href: 'https://discord.gg/34CdSnXe',
                icon: '/linkout.svg',
                text: 'Join Discord',
            ...
            },
            {
                ...
                icon: '/download.svg',
                text: 'Download Pulse Camera',
                ariaLabel: 'Download Pulse Camera (new window)',
            },
            {
                ...
                icon: '/download.svg',
                text: 'Download Pulse Glass',
                ariaLabel: 'Download Pulse Glass (new window)',
            },
        ],
        []
    );
    ...
    return (
        ...
        >
            <header className={`group relative h-6 flex shrink-0 items-center justify-between`}>
                {isCollapsed ? (
                    <Link href="https://pulse.com" target="_blank" rel="noopener noreferrer" className="flex items-center">
                    ...
                    <>
                        <Link href="https://pulse.com" target="_blank" rel="noopener noreferrer" className="flex items-center">
                            {isCollapsed ? (
                                ...
                                <span className="mx-3 shrink-0 text-lg font-bold text-gray-900" style={getUniformTextStyle()}>
                                    Pulse
                                </span>
                            )}
                        </Link>
                ...
                {!isCollapsed && hasApiKey !== null && (
                    <div className="px-2.5 py-2 text-center">
                        <span className={`text-xs px-2 py-1 rounded-full ${hasApiKey ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                            {hasApiKey ? 'Local running' : 'Pulse Free System'}
                        </span>
                    </div>
                )}
